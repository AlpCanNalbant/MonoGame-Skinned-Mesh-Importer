...
namespace Liru3D.Animations
{
    ...
    public sealed class ChannelComponent<T> where T : struct
    {
        #region Properties
        /// <summary> Every keyframe in this channel. </summary>
        public readonly IReadOnlyList<Keyframe<T>> Keyframes;
        ...
        public static ChannelComponent<T> Clone(ChannelComponent<T> other)
        {
            List<Keyframe<T>> keyframes = new();
            for (int i = 0; i < other.Keyframes.Count; ++i)
            {
                keyframes.Add(other.Keyframes[i]);
            }
        ...
        private static float MeasureFrameDistance(AnimationPlayer animationPlayer, float firstTick, float secondTick)
        ...
        /// <param name="secondTick"> The tick that comes "second" in the playback. </param>
        /// <returns> The distance in ticks between the two frames. </returns>
        private static int MeasureFrameDistance(AnimationPlayer animationPlayer, int firstTick, int secondTick) => (int)MeasureFrameDistance(animationPlayer, (float)firstTick, secondTick);
        #endregion
    }
}
